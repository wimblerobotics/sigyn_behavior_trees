...
/**
 * @class sigyn_behavior_trees::MoveAShortDistanceAheadActionServer
 * @brief An action server for the MoveAShortDistanceAhead action
 */
class MoveAShortDistanceAheadActionServer : public rclcpp::Node {
 public:
  ...
  using GoalHandleMoveAShortDistanceAhead =
      rclcpp_action::ServerGoalHandle<MoveAShortDistanceAhead>;

  /**
   * @brief A constructor for sigyn_behavior_trees::MoveAShortDistanceAheadServer
   * @param options Additional options to control creation of the node.
   */
  explicit MoveAShortDistanceAheadActionServer(
    ...
    auto handle_cancel =
        [this](const std::shared_ptr<GoalHandleMoveAShortDistanceAhead> goal_handle) {
 ...
 private:
  ...
  void execute(const std::shared_ptr<GoalHandleMoveAShortDistanceAhead> goal_handle) {
    ...
    RCLCPP_INFO(this->get_logger(), "[execute] Received goal request with distance %4.3f",
                goal->distance);
    auto result = std::make_shared<MoveAShortDistanceAhead::Result>();
